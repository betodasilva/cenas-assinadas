<?php
/**
 * Rango functions and definitions
 *
 ...
 */
/**
 * Rango only works in WordPress 4.8 or later.
 */
...
function ftc_custom_sidebar_form() {
    ...
    <form action="<?php echo admin_url('widgets.php'); ?>" method="post" id="ftc-form-add-sidebar">
        <input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_html_e('Custom Sidebar Name', 'rango') ?>" />
        <button class="button-primary" id="ftc-add-sidebar"><?php esc_html_e('Add Sidebar', 'rango') ?></button>
    </form>
    <?php
}
...
function ftc_setup() {
     ...
     * If you're building a theme based on Rango, use a find and replace
     * to change 'rango' to the name of your theme in all the template files.
     */
    load_theme_textdomain('rango');
    ...
    /* Translation */
    load_theme_textdomain('rango', get_template_directory() . '/languages');
    ...
    register_nav_menus(array(
        'primary' => __('Primary Navigation', 'rango'),
        'vertical' => __('Vertical Navigation', 'rango'),
    ));
    ...
    $starter_content = array(
        ...
        'attachments' => array(
            'image-espresso' => array(
                'post_title' => _x('Espresso', 'Theme starter content', 'rango'),
                'file' => 'assets/images/espresso.jpg', // URL relative to the template directory.
            ),
            'image-sandwich' => array(
                'post_title' => _x('Sandwich', 'Theme starter content', 'rango'),
                'file' => 'assets/images/sandwich.jpg',
            ),
            'image-coffee' => array(
                'post_title' => _x('Coffee', 'Theme starter content', 'rango'),
        ...
        'nav_menus' => array(
            ...
            'social' => array(
                'name' => __('Social Links Menu', 'rango'),
                'items' => array(
                    'link_yelp',
                    'link_facebook',
                    'link_twitter',
    ...
    /**
     * Filters Rango array of starter content.
     *
     * @since Rango 1.1
     *
...
function ftc_content_width() {
    ...
    /**
     * Filter Rango content width of the theme.
     *
     * @since Rango 1.0
     *
...
function ftc_fonts_url() {
     ...
     */
    $dosis = _x('on', 'Lato font: on or off', 'rango');
...
function ftc_excerpt_more($link) {
    ...
    $link = sprintf('<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>', esc_url(get_permalink(get_the_ID())),
        /* translators: %s: Name of current post */ sprintf(__('Continue reading<span class="screen-reader-text"> "%s"</span>', 'rango'), get_the_title(get_the_ID()))
    );
    return ' &hellip; ' . $link;
}
...
function ftc_scripts() {
    ...
    if (has_nav_menu('top')) {
        ...
        $ftc_l10n['expand'] = __('Expand child menu', 'rango');
        $ftc_l10n['collapse'] = __('Collapse child menu', 'rango');
...
if (!function_exists('ftc_get_search_form_by_category')) {
    ...
    function ftc_get_search_form_by_category() {
        ...
        if ($search_for_product) {
            ...
            $post_type = 'product';
            $orderby = 'relevance';
            $placeholder_text = esc_html__('Search ...', 'rango');
        } else {
            $taxonomy = 'category';
            $post_type = 'post';
            $placeholder_text = esc_html__('Search', 'rango');
        ...
        $options = '<option value="">' . esc_html__('All categories', 'rango') . '</option>';
...
if (!function_exists('ftc_ajax_search')) {
    ...
    function ftc_ajax_search() {
        ...
        if ($results->have_posts()) {
            ...
            if (isset($results->post_count, $results->found_posts) && $results->found_posts > $results->post_count) {
                $view_all_text = sprintf(esc_html__('View all %d results', 'rango'), $results->found_posts);
...
if (!function_exists('ftc_breadcrumbs')) {
    ...
    function ftc_breadcrumbs() {
        ...
        if (!empty($front_id)) {
            ...
            $home = esc_html__('Home', 'rango');
        }
        $ar_title = array(
            'search' => esc_html__('Search results for ', 'rango')
            , '404' => esc_html__('Error 404', 'rango')
            , 'tagged' => esc_html__('Tagged ', 'rango')
            , 'author' => esc_html__('Articles posted by ', 'rango')
            , 'page' => esc_html__('Page', 'rango')
            , 'portfolio' => esc_html__('Portfolio', 'rango')
        );
...
function ftc_breadcrumbs_title($show_breadcrumb = false, $show_page_title = false, $page_title = '', $extra_class_title = '') {
    ...
    if ($show_breadcrumb || $show_page_title) {
        ...
        echo '<div class="ftc-breadcrumb" ' . $style . '><div class="container"><div class="ftc-breadcrumb-title">';
        if ($show_page_title) {
...
function ftc_register_required_plugins() {
    ...
    $plugins = array(
        ...
        , array(
            ...
            'source' => $plugin_dir_path . 'mega_main_menu.zip', // The plugin source.
            'required' => false, // If false, the plugin is only 'recommended' instead of required.
