...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'backup':
					...
					if(!isset($backup['backup_log'])) {
						$log = esc_html__('No backups yet', 'rango');
					} else {
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ', 'rango').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button" title="'.esc_html__('Backup Options', 'rango').'">'.esc_html__('Backup Options', 'rango').'</a>';
					$output .= '<a href="#" id="of_restore_button" class="button" title="'.esc_html__('Restore Options', 'rango').'">'.esc_html__('Restore Options', 'rango').'</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button" title="'.esc_html__('Restore Options', 'rango').'">'.esc_html__('Import Options', 'rango').'</a>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">'.esc_html__('Upload', 'rango').'</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'.esc_html__('Remove', 'rango').'</span>';
		}
		else 
		{
