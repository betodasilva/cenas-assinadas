...
if( !class_exists('Ftc_Grid_List') && ftc_has_woocommerce() ){
	class Ftc_Grid_List{
		function __construct(){
			...
			$this->settings = array(
				array(
					'name' => esc_html__( 'Default catalog view', 'rango' ),
					'type' => 'title',
					'id' => 'ftc_glt_options'
				),
				array(
					'name' 		=> esc_html__( 'Catalog view', 'rango' ),
					'desc_tip' 	=> '',
					'id' 		=> 'ftc_enable_glt',
					'type' 		=> 'checkbox',
					'desc' 		=> esc_html__('Display option to show product in grid or list view', 'rango'),
					'default' 	=> 'yes'
				),
				array(
					'name' 		=> esc_html__( 'Default catalog view', 'rango' ),
					'desc_tip' 	=> esc_html__( 'Display products in grid or list view by default', 'rango' ),
					'id' 		=> 'ftc_glt_default',
					'type' 		=> 'select',
					'options' 	=> array(
						'grid'  => esc_html__('Grid', 'rango'),
						'list' 	=> esc_html__('List', 'rango')
					)
				),
		...
		function gridlist_toggle_button() {
			?>
				<nav class="grid_list_nav">
					<a href="#" id="grid" title="<?php esc_html_e('Grid view', 'rango'); ?>">&#8862; <span><?php esc_html_e('Grid view', 'rango'); ?></span></a><a href="#" id="list" title="<?php esc_html_e('List view', 'rango'); ?>">&#8863; <span><?php esc_html_e('List view', 'rango'); ?></span></a>
				</nav>
			<?php
		}
