...
function ftc_product_get_availability(){
    ...
    if ( ! $product->is_in_stock() ) {
        $availability = esc_html__( 'Out of stock', 'rango' );
    } elseif ( $product->managing_stock() && $product->is_on_backorder( 1 ) ) {
        $availability = esc_html__( 'Available on backorder', 'rango' );
...
function ftc_template_loop_categories(){
 global $product;        
    $categories_label = esc_html__('Categories: ', 'rango');
...
function ftc_shop_top_content_widget_area_button() {
    ...
    if (is_active_sidebar('product-category-top-content') && $smof_data['ftc_prod_cat_top_content']) {
        ?>
        <div class="prod-cat-show-top-content-button"><a href="#"><?php esc_html_e('Filter', 'rango') ?></a></div>
        <?php
    }
}
...
function ftc_template_single_navigation() {
    ...
    <div class="social-sharing">
        <div class="print">
            <a href="javascript:window.print()" rel="nofollow"><i class="fa fa-print"></i><?php esc_html_e('Print', 'rango') ?></a>
        </div>
        <div class="email">
            <a href="mailto:?subject=<?php echo esc_attr(sanitize_title(get_the_title())); ?>&amp;body=<?php echo esc_url(get_permalink()); ?>">
                <i class="fa fa-envelope"></i>
    <?php esc_html_e('Email to a Friend', 'rango') ?>
            </a>
        </div>
...
function ftc_template_after_single_social_sharing() {
    ...
    function ftc_template_single_meta() {
       ...
       if ($smof_data['ftc_prod_cat']) {
            echo wc_get_product_category_list($product->get_id(), ', ', '<div class="caftc-link"><span>' . _n('Category:', 'rango', count($product->get_category_ids()), 'rango') . ' ', '</span><span class="cat-links"></span></div>');
        }
        if ($smof_data['ftc_prod_tag']) {
            echo wc_get_product_tag_list($product->get_id(), ', ', '<div class="tags-link"><span>' . _n('Tag:', 'Tags:', count($product->get_tag_ids()), 'rango') . ' ', '</span><span class="tag-links"></span></div>');
        }
    ...
    function ftc_template_single_sku() {
        ...
        if (wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type('variable') )) {
            echo '<div class="sku-wrapper product_meta">' . esc_html__('Sku: ', 'rango') . '<span class="sku" itemprop="sku">' . (( $sku = $product->get_sku() ) ? $sku : esc_html__('N/A', 'rango')) . '</span></div>';
        }
    }
    ...
    function ftc_template_single_availability() {
        ...
        $product_stock = ftc_product_get_availability();
        $availability_text = empty($product_stock['availability']) ? esc_html__('In Stock', 'rango') : esc_attr($product_stock['availability']);
        ?>	
 ...
 * End tag:   	10005
 * ************************************************************ */
add_action('woocommerce_after_shop_loop_item_title', 'ftc_template_loop_add_to_cart', 10002);
...
function ftc_product_group_button_start() {
    ...
    if ($smof_data['ftc_effect_hover_product_style'] != 'style-3') {
        ...
        remove_action('woocommerce_after_shop_loop_item_title', 'ftc_template_loop_add_to_cart', 10002);
    }
...
if (class_exists('YITH_WCWL')) {
    ...
    function ftc_add_wishlist_button_to_product_list() {
        ...
        . '" class="add_to_wishlist wishlist" ><i class="fa fa-heart"></i><span class="ftc-tooltip button-tooltip">' . esc_html__('Wishlist', 'rango') . '</span></a>';
        ...
        echo '<a href="' . esc_url($wishlist_url) . '"><i class="fa fa-heart-o"></i><span class="ftc-tooltip button-tooltip">' . esc_html__('Wishlist', 'rango') . '</span></a>';
        echo '</span>';
    ...
    add_action('woocommerce_after_shop_loop_item_title', 'ftc_add_wishlist_button_to_product_list', 10001);
    add_action('woocommerce_after_shop_loop_item', 'ftc_add_wishlist_button_to_product_list', 70);
}
...
function ftc_cart_continue_shopping_button() {
    echo '<a href="' . esc_url(wc_get_page_permalink('shop')) . '" class="button button-secondary">' . esc_html__('Continue Shopping', 'rango') . '</a>';
}
?>