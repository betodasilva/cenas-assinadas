...
if (!function_exists('ftc_tiny_account')) {
    ...
    function ftc_tiny_account() {
        ...
        <div class="ftc-account">
            <div class="ftc_login">
                <?php if (!$_user_logged): ?>
                    <a  class="login" href="<?php echo esc_url($login_url); ?>" title="<?php esc_html_e('Login', 'rango'); ?>"><span><?php esc_html_e('Login', 'rango'); ?></span></a>
                    / 
                    <a class="ftc_sign_up" href="<?php echo esc_url($register_url); ?>" title="<?php esc_html_e('Create New Account', 'rango'); ?>"><span><?php esc_html_e('Sign up', 'rango'); ?></span></a>
                <?php else: ?>
                    <a class="my-account" href="<?php echo esc_url($profile_url); ?>" title="<?php esc_html_e('My Account', 'rango'); ?>"><span><?php esc_html_e('My Account', 'rango'); ?></span></a> /
                    <a class="log-out" href="<?php echo esc_url($logout_url); ?>" title="<?php esc_html_e('Logout', 'rango'); ?>"><span><?php esc_html_e('Logout', 'rango'); ?></span></a>
                <?php endif; ?>
            </div>
            <?php if (!$_user_logged): ?>
                <div class="ftc_account_form dropdown-container">
                        <form name="ftc-login-form" class="ftc-login-form" action="<?php echo esc_url(wp_login_url()); ?>" method="post">
                            ...
                            <p class="login-username">
                                <label><?php esc_html_e('Username', 'rango'); ?></label>
                                <input type="text" name="log" class="input" value="" size="20" autocomplete="off">
                            </p>
                            <p class="login-password">
                                <label><?php esc_html_e('Password', 'rango'); ?></label>
                                <input type="password" name="pwd" class="input" value="" size="20">
                            </p>

                            <p class="login-submit">
                                <input type="submit" name="wp-submit" class="button-secondary button" value="<?php esc_html_e('Login', 'rango'); ?>">
                                <input type="hidden" name="redirect_to" value="<?php echo esc_url($redirect_to); ?>">
                            </p>

                        </form>

                        <p class="ftc_forgot_pass"><a href="<?php echo esc_url(wp_lostpassword_url()); ?>" title="<?php esc_html_e('Forgot Your Password?', 'rango'); ?>"><?php esc_html_e('Forgot Your Password?', 'rango'); ?></a></p>
                </div>
            <?php endif; ?>
        </div>
...
if (!function_exists('ftc_tiny_cart')) {
    ...
    function ftc_tiny_cart() {
        ...
        <div class="ftc-shoppping-cart">
            <a class="ftc_cart" href="<?php echo esc_url($cart_url); ?>" title="<?php esc_html_e('View your shopping bag', 'rango'); ?>">
            <span class="test"><?php esc_html_e(' SHOPPPING CART', 'rango'); ?></span>
                <span class="cart-number<?php if ($cart_number > 0) {
            echo esc_html(" no-bracket");
        } ?>"><?php if ($cart_number > 0) {
            if ($cart_number > 1) {
                echo esc_html($cart_number);
                esc_html_e(' Item(s) - ', 'rango');
            } else {
                echo esc_html($cart_number);
                esc_html_e(' Item(s) - ', 'rango');
            }
            ...
            <div class="ftc_shopping_form dropdown-container">
                    <?php if ($cart_empty): ?>
                    <label><?php esc_html_e('Your shopping cart is empty', 'rango'); ?></label>
                            <?php else: ?>
            ...
            foreach ($cart as $cart_item_key => $cart_item):
                ...
                <?php echo apply_filters('woocommerce_cart_item_remove_link', sprintf('<a href="%s" class="remove" title="%s" data-key="%s">&times;</a>', esc_url(WC()->cart->get_remove_url($cart_item_key)), esc_html__('Remove this item', 'rango'), $cart_item_key), $cart_item_key); ?>
                                </div>
                            </li>
            ...
            <?php endforeach; ?>
                    ...
                    <div class="ftc_cart_check">
                        <div class="total"><span class="total-title"><?php esc_html_e('Subtotal :', 'rango'); ?></span><?php echo WC()->cart->get_cart_subtotal(); ?> </div>

                        <a href="<?php echo esc_url($cart_url); ?>" class="button view-cart"><?php esc_html_e('View cart', 'rango'); ?></a>
                        <a href="<?php echo esc_url($checkout_url); ?>" class="button checkout button-secondary"><?php esc_html_e('Checkout', 'rango'); ?></a>
                    </div>
        <?php endif; ?>
...
function ftc_tini_wishlist() {
    ...
    <a title="<?php esc_html_e('Wishlist', 'rango'); ?>" href="<?php echo esc_url($wishlist_page); ?>" class="tini-wishlist">
    <?php esc_html_e('Wishlist', 'rango'); ?> <?php echo '(' . ($count > 0 ? zeroise($count, 2) : '0') . ')'; ?>
    </a>
