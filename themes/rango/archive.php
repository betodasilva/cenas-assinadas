...
switch( true ){
	case is_day():
		$page_title = esc_html__( 'Day: ', 'rango' ) . get_the_date();
	break;
	case is_month():
		$page_title = esc_html__( 'Month: ', 'rango' ) . get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'rango' ) );
	break;
	case is_year():
		$page_title = esc_html__( 'Year: ', 'rango' ) . get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'rango' ) );
	break;
	case is_search():
		$page_title = esc_html__( 'Search Results for: ', 'rango' ) . get_search_query();
	break;
	case is_tag():
		$page_title = esc_html__( 'Tag: ', 'rango' ) . single_tag_title( '', false );
	break;
	case is_category():
		$page_title = esc_html__( 'Category: ', 'rango' ) . single_cat_title( '', false );
	break;
	case is_404():
		$page_title = esc_html__( 'OOPS! FILE NOT FOUND', 'rango' );
	break;
	default:
		$page_title = esc_html__( 'Archives', 'rango' );
	break;
}
