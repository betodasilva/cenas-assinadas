...
<div <?php wc_product_cat_class('category-slider product-category product', $category); ?>>
    ...
    <div class="item-description">
        ...
        <?php
        if ($category->count > 0) {
            echo apply_filters('woocommerce_subcategory_count_html', '<span class="count-product-category">' . sprintf(_n('%s Product', '%s Products', $category->count, 'rango'), $category->count) . '</span>', $category);
        }
        ?>
        ...
        <?php endif; ?>
            <a href="<?php echo get_term_link($category->slug, 'product_cat'); ?>" class="button category-button"><?php esc_html_e('Shop Now', 'rango'); ?></a>
    </div>
